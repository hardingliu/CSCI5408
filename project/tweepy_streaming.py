...
class TweetStreamListener(tweepy.StreamListener):
    ...
    def on_status(self, status):
        ...
        data = self.get_tweet(status)
        # data_dict = {}
        # data_dict["tweet"] = data[0]
        # data_dict["coordinates"] = str(data[1])
        # data_dict["created_at"] = str(data[2])
        # data_dict["retweet_count"] = data[3]
        # data_dict["favorite_count"] = data[4]
        # data = json.dumps(data_dict) + '\n'
        with open('info.csv', 'a', newline='') as f:
            writer = csv.writer(f)
            writer.writerow(data)
            self.client_socket.send((data[0] + "\n").encode('utf-8'))
            return True
    ...
    def get_tweet(self, tweet):
        ...
        return [self.clean_str(text), str(tweet.user.location), str(tweet.coordinates)]
